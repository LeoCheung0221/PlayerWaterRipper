...
public class RippleAnimationView extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        LayoutParams rippleParams = new LayoutParams(UIUtils.getInstance().getWidth(radius + strokeWidth), UIUtils.getInstance().getWidth(radius + strokeWidth));
        rippleParams.addRule(CENTER_IN_PARENT, TRUE);
        float maxScale = 10;
        //UIUtils.getInstance().displayMetricsWidth / (float) ( (UIUtils.getInstance().getWidth(radius + strokeWidth)));
//        延迟时间
